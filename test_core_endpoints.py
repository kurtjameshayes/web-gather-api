...
def _wire_mongo(mock_mongo: MagicMock) -> Dict[str, MagicMock]:
    ...
    db.__getitem__.return_value = user_collection
...
def test_ingest_overwrite_deletes_only_same_source_url(client, mock_clients) -> None:
    dbs = _wire_mongo(mock_clients["mongo"])
    user_collection = dbs["user_collection"]
    documents_collection = dbs["documents_collection"]
    source_url = "https://example.com/privacy"

    user_collection.count_documents.return_value = 2
    mock_clients["firecrawl"].crawl.return_value = [
        {"url": source_url, "title": "Privacy Policy", "markdown": "Policy body"}
    ]
    ...
    source_filter = {"source_url": source_url}
    user_collection.count_documents.assert_called_once_with(source_filter)
    user_collection.delete_many.assert_called_once_with(source_filter)
