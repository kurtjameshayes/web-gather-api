...
def test_get_category_mapping_rejects_dangerous_query(client, mock_coll) -> None:
    """NoSQL injection operators must be rejected before collection access."""
...
def test_delete_category_mapping_rejects_dangerous_query(client, mock_coll) -> None:
    """NoSQL injection operators must be rejected before delete_many."""
    query = urllib.parse.quote('{"$where": "1 == 1"}')
