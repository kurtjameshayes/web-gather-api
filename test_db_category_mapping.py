...
def test_get_category_mapping_rejects_dangerous_query_operator(client, mock_coll) -> None:
    """GET /category-mapping must reject Mongo operators before find()."""
    response = client.get(
        "/category-mapping",
        query_string={"query": '{"$where": "this.policy_categories.length > 0"}'},
    )

    assert response.status_code == 400
    assert "operator" in response.get_json()["error"].lower()
...
def test_delete_category_mapping_rejects_dangerous_query_operator(client, mock_coll) -> None:
    """DELETE /category-mapping must reject Mongo operators before delete_many()."""
    response = client.delete(
        "/category-mapping",
        query_string={"query": '{"$where": "this.policy_categories.length > 0"}'},
    )
