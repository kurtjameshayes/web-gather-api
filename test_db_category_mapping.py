...
def test_get_category_mapping_rejects_dangerous_query_operator(client, mock_coll) -> None:
    """NoSQL injection operators must be rejected before category mapping lookup."""
...
def test_delete_category_mapping_rejects_dangerous_query_operator(client, mock_coll) -> None:
    """NoSQL injection operators must be rejected before category mapping delete."""
    query = urllib.parse.quote('{"$regex": ".*"}')
