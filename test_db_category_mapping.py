...
def test_get_category_mapping_rejects_dangerous_query_operator(client, mock_coll) -> None:
    """NoSQL operators must not reach category-mapping find filters."""
    response = client.get(
        "/category-mapping",
        query_string={"query": json.dumps({"$regex": ".*"})},
    )
...
def test_delete_category_mapping_rejects_dangerous_query_operator(client, mock_coll) -> None:
    """NoSQL operators must not reach category-mapping delete filters."""
    response = client.delete(
        "/category-mapping",
