...
def reset_app_api_key(monkeypatch: Any):
    """Reset APP_API_KEY-backed decorator state before and after a test."""
...
def test_require_api_key_allows_unconfigured_app_key(reset_app_api_key: None) -> None:
    app = Flask(__name__)
...
def test_require_api_key_rejects_missing_and_invalid_keys(monkeypatch: Any, reset_app_api_key: None) -> None:
    app = Flask(__name__)
...
def test_require_api_key_accepts_valid_key(monkeypatch: Any, reset_app_api_key: None) -> None:
    app = Flask(__name__)
    monkeypatch.setenv("APP_API_KEY", "secret")
    security.init_app_api_key()
...
def test_require_api_key_async_enforces_app_key(monkeypatch: Any, reset_app_api_key: None) -> None:
    app = Flask(__name__)
    monkeypatch.setenv("APP_API_KEY", "secret")
    security.init_app_api_key()
